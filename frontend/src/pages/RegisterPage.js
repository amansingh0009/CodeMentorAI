...
const passwordInputClass = 'h-12 w-full rounded-2xl border border-slate-700 bg-slate-950/70 px-4 text-base text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/10';
...
const RegisterPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Unable to register at this time.');
    }
  };
  ...
  return (
    <div className="mx-auto max-w-[500px] rounded-3xl border border-slate-800 bg-slate-900/80 px-8 py-10 shadow-2xl shadow-slate-950/30 backdrop-blur-xl">
      ...
      {success && <div className="mt-7 rounded-2xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">{success}</div>}
      <form onSubmit={handleSubmit} autoComplete="off" className="mt-9 space-y-5">
        <input
          value={username}
      ...
      <p className="mt-8 text-center text-sm text-slate-400">
        Have an account? <Link to="/login" className="text-cyan-300 hover:text-cyan-200">Sign In</Link>
      </p>
      <div className="mt-9 space-y-4 text-center">
        <p className="text-sm text-slate-500">or continue with</p>
        <div className="flex justify-center gap-5">
          {[FaGoogle, FaGithub, FaApple].map((Icon, index) => (
            <button
              key={index}
              type="button"
